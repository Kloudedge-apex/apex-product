...
for (const literal of [
  ...
  "phase:",
  "PREPARE WORKFORCE OS PUBLIC DOMAINS",
...
for (const literal of [
  ...
  'PREPARE_CONFIRMATION_PHRASE="PREPARE WORKFORCE OS PUBLIC DOMAINS"',
  'BIND_CONFIRMATION_PHRASE="PROMOTE WORKFORCE OS PUBLIC DOMAINS"',
...
if ((promotion.match(/az containerapp hostname add/gu) ?? []).length !== 2 ||
  (promotion.match(/az containerapp hostname bind/gu) ?? []).length !== 2 ||
  (promotion.match(/az containerapp hostname delete/gu) ?? []).length !== 4) {
  fail("promotion prepare, bind, and rollback command counts are not exact");
}
