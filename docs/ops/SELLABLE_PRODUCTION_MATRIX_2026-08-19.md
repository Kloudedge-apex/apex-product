...
| Review research, score rationale, and evidence | **SOURCE PASS.** Lead research briefs, score breakdowns, intent signals, and recent evidence are derived only when attributable data exists; missing facts remain unavailable rather than fabricated. The evidence writer admits only absolute HTTP(S) citations without embedded credentials, strict real `yyyy-mm-dd` calendar dates, and confidence probabilities in `(0, 1]`; read-side presentation independently revalidates the same contract. Evidence timelines use the signal's occurrence date and expose a structured reviewer source URL instead of presenting the later persistence time as the event time. Extracted signals count only after the evidence ledger confirms a durable create or an idempotent existing row. Production startup and release verification require `EVIDENCE_LEDGER_ENABLED=true`; if every extracted signal misses persistence, research fails before the approval prompt instead of claiming success. | **LIVE PROOF REQUIRED.** Verify representative leads contain attributable, directly reviewable citations and truthful occurrence dates, and that evidence-poor leads refuse unsupported personalization. Exercise a deliberate malformed citation and ledger-write failure and confirm both fail before approval. |
| Approve or reject a run before drafting | **SOURCE PASS.** Starting a cost-bearing pipeline, changing the current ICP, and approving or rejecting a run all require synchronized owner/admin/manager authority with a signed Clerk-role veto. Tenant-scoped compare-and-set decisions, server-derived reviewer identity, explicit fail-closed UI capabilities, and duplicate/stale conflict handling are implemented. Regular members retain read access to run history and ICP state but cannot mutate workflow execution. | **LIVE PROOF REQUIRED.** Exercise authorized and unauthorized sessions against the migrated runtime. |
