...
# Separately authorized start or restart of the production Azure mutation-
# authority credential-drain window. The live read-only audit must already
# prove structural exclusivity. This script writes only the fixed zero-byte
# checkpoint metadata and never deletes a blob or uses account-key auth.
...
CREATE_CONFIRMATION="CREATE WORKFORCE OS AUTHORITY DRAIN CHECKPOINT"
RESET_CONFIRMATION="RESET WORKFORCE OS AUTHORITY DRAIN CHECKPOINT"
MODE=""
CONFIRMATION=""
APPLY="false"

usage() {
  echo "Usage: $0 --apply --mode create|reset --confirmation '<exact phrase>'" >&2
}
...
if [[ "${APPLY}" != "true" ]]; then
  echo "ERROR: explicit checkpoint mutation authority is absent" >&2
  exit 1
fi
case "${MODE}" in
  create)
    [[ "${CONFIRMATION}" == "${CREATE_CONFIRMATION}" ]] || {
      echo "ERROR: create confirmation phrase does not match" >&2
      exit 1
    }
    ;;
  reset)
    [[ "${CONFIRMATION}" == "${RESET_CONFIRMATION}" ]] || {
      echo "ERROR: reset confirmation phrase does not match" >&2
      exit 1
    }
    ;;
  *)
...
jq -e \
  ...
  --arg blob "${BLOB}" '
    ...
    and all(
      ...
      .target == "stateStorage"
      and (
        .code == "credential-drain-checkpoint-missing"
        or .code == "credential-drain-checkpoint-invalid"
        or .code == "credential-drain-structure-mismatch"
        or .code == "credential-drain-window-open"
      )
    )
  ' >/dev/null <<<"${audit_json}"
if [[ "${MODE}" == "create" ]]; then
  jq -e 'any(.findings[]; .code == "credential-drain-checkpoint-missing")' \
    >/dev/null <<<"${audit_json}"
fi
...
backend_release_client_id="$(jq -er '.identities.backendRelease.clientId' <<<"${audit_json}")"
console_release_client_id="$(jq -er '.identities.consoleRelease.clientId' <<<"${audit_json}")"
jq -e \
  --arg subscription "${SUBSCRIPTION_ID}" \
  --arg backend "${backend_release_client_id}" \
  --arg console "${console_release_client_id}" '
  (.id | ascii_downcase) == ($subscription | ascii_downcase)
  and .state == "Enabled"
  and .user.type == "servicePrincipal"
  and (
    (.user.name | ascii_downcase) == ($backend | ascii_downcase)
    or (.user.name | ascii_downcase) == ($console | ascii_downcase)
  )
...
runtime_dir="$(mktemp -d "${TMPDIR:-/tmp}/workforce-authority-drain.XXXXXX")"
...
if [[ "${MODE}" == "create" ]]; then
  ...
  checkpoint_before="$(az storage blob show \
    --subscription "${SUBSCRIPTION_ID}" \
    --account-name "${STORAGE_ACCOUNT}" \
    --container-name "${CONTAINER}" \
    --name "${BLOB}" \
    ...
    --output json \
    --only-show-errors)"
  checkpoint_etag="$(jq -er '.properties.etag | select(test("^\\\".+\\\"$"))' \
    <<<"${checkpoint_before}")"
  jq -e '.properties.contentLength == 0' >/dev/null <<<"${checkpoint_before}"
