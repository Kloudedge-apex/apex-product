...
export class WebSearchTool implements Tool {
  name = "web_search";
  description =
    "Search the live web for information. Returns titles, URLs, snippets and content for the top results. Returns an explicit failure when no provider is configured or the provider is unavailable.";
  parameters = {
  ...
  async execute(params: Record<string, unknown>, _context: ToolContext): Promise<ToolResult> {
    ...
    // Fall back to Serper, which production already supports. This keeps live
    // trigger extraction available when Tavily is not configured.
    ...
    return this.failure(
      "Web search is unavailable because no live provider is configured (TAVILY_API_KEY or SERPER_API_KEY).",
    );
  }
  ...
  private async searchWithTavily(
    ...
    try {
      ...
      return this.failure(
        `Tavily web search failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async searchWithSerper(
    ...
    try {
      ...
      return this.failure(
        `Serper web search failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
